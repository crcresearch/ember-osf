    ...
    save(options = {
        ...
        this.eachRelationship((rel) => {
            ...
            if (relation.hasData && relation.hasLoaded) {
                var canonicalIds = relation.canonicalMembers.list.map(member => member._record.get('id'));
                var currentIds = relation.members.list.map(member => member._record.get('id'));
                var changes = {
                    create: relation.members.list.filter(m => m._record.get('isNew')),
                    add: relation.members.list.filter(m => !m._record.get('isNew') && canonicalIds.indexOf(m._record.get('id')) === -1),
                    remove: relation.canonicalMembers.list.filter(m => currentIds.indexOf(m._record.get('id')) === -1)
                };
