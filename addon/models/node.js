...
export default OsfModel.extend(FileItemMixin, {
    ...
    save() {
        ...
        this.set(
            ...
            contributors.members.list.filter(m => !m._record.get('isNew') && Object.keys(m._record.changedAttributes()).length > 0)
        );
        // Need to included created contributors even in relationship
        // hasLoaded is false
        this.set(
            '_dirtyRelationships.contributors.create',
            contributors.members.list.filter(m => m._record.get('isNew'))
        );
